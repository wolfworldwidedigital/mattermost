...
This command should be run using a database configuration DSN.`,
  ...
  $ Cusmato db init --config postgres://localhost/mattermost

  # or you can use the MM_CONFIG environment variable
  $ MM_CONFIG=postgres://localhost/mattermost Cusmato db init

  # and you can set a custom defaults file to be loaded into the database
  $ MM_CUSTOM_DEFAULTS_PATH=custom.json MM_CONFIG=postgres://localhost/mattermost Cusmato db init`,
	Args: cobra.NoArgs,
	RunE: initDbCmdF,
}
...
var ResetCmd = &cobra.Command{
	...
	Long:  "Completely erases the database causing the loss of all data. This will reset Cusmato to its initial state.",
	RunE:  resetCmdF,
}
